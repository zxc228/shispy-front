...
export default function JoinPage() {
  ...
  const totalTon = useMemo(() => {
    const sum = selectedIds
      .map((tid) => inventory.find((t) => t.id === tid)?.priceTON ?? 0)
      .reduce((a, b) => a + b, 0)
...
function TreasureCard({ variant, treasure, selected, onToggle }) {
  ...
  if (variant === 'empty') {
    return (
      <div className={`${baseSize} rounded-[10px] border border-neutral-700 bg-neutral-800/30`} />
    )
  }
  ...
  return (
    ...
    >
      {treasure?.image ? (
        ...
        <img src={treasure.image} alt="Treasure" className="w-full h-full object-cover rounded-[10px]" />
      ) : (
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="inline-flex items-center gap-2">
              ...
              <div className="h-8 px-2.5 py-2 bg-black rounded-xl inline-flex items-center gap-1">
                ...
                <span className="text-white text-lg font-bold">{totalTon.toFixed(2)} TON</span>
              </div>
            </div>
          </div>
          <div className="mt-1 text-neutral-700 text-sm">Fixed game commission of 0.2 TON</div>
