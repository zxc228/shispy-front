...
import { createBattle } from '../../shared/api/lobby.api'
...
function TreasureCard({ variant, treasure, selected, onToggle }) {
  ...
  return (
    <button
      ...
      onClick={onToggle}
      className={`relative ${baseSize} rounded-[10px] border border-zinc-500 overflow-hidden transition-all duration-200 flex flex-col ${selected ? 'animate-[bounce_0.5s_ease-out] scale-105' : 'hover:scale-105 active:scale-95'}`}
    >
      {treasure?.image ? (
